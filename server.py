...
def analyze_security_risk(prompt_text, user_id):
    # 1. WATCHLIST (Check User's AND Manager's list)
    try:
        conn = get_db_connection(); cur = conn.cursor()
        # Get keywords for user OR their manager
        cur.execute("""
            SELECT keyword FROM watchlist_v7 
            WHERE user_id = %s OR user_id = (SELECT manager_id FROM users_v7 WHERE id = %s)
        """, (user_id, user_id))
        rows = cur.fetchall(); cur.close(); conn.close()
        for row in rows:
            if row[0].lower() in prompt_text.lower():
                return 100, f"Banned keyword detected: '{row[0]}'", "This word is on your team's blocklist."
    except: pass
    ...
    except: return 50, "AI Analysis Failed", "Try again."
...
def load_user(user_id):
    try:
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute("SELECT * FROM users_v7 WHERE id = %s", (user_id,))
...
NAVBAR_TEMPLATE = """<nav x-data="{ open: false }" class="fixed w-full z-50 glass border-b border-slate-800 transition-all duration-300"><div class="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center"><a href="/" class="flex items-center gap-3">{{ logo|safe }}<span class="text-xl font-bold tracking-tight text-white">VIGIL</span></a><div class="hidden md:flex gap-8 text-sm font-medium text-slate-400 items-center">{% if current_user.is_authenticated %}<a href="/guide" class="text-indigo-400 font-bold hover:text-indigo-300 transition flex items-center gap-1"><span>📚</span> How to Install</a><a href="/dashboard" class="text-white hover:text-indigo-400 transition">Dashboard</a><a href="/logout" class="px-4 py-2 bg-slate-800 text-white rounded-lg font-bold hover:bg-slate-700 transition">Log Out</a>{% else %}<a href="/guide" class="text-indigo-400 font-bold hover:text-indigo-300 transition flex items-center gap-1"><span>📚</span> How to Install</a><a href="/login" class="text-slate-300 hover:text-white transition">Log in</a><a href="/register" class="px-4 py-2 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-500 transition">Get Started</a>{% endif %}</div><button @click="open = !open" class="md:hidden text-slate-300 focus:outline-none"><svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg></button></div><div x-show="open" @click.away="open = false" class="md:hidden bg-slate-900 border-b border-slate-800 absolute w-full left-0 top-20 shadow-2xl"><div class="flex flex-col p-6 gap-4 text-center">{% if current_user.is_authenticated %}<a href="/dashboard" class="text-indigo-400 font-bold py-2">Go to Dashboard</a><a href="/logout" class="text-slate-400 py-2">Log Out</a>{% else %}<a href="/login" class="text-indigo-400 font-semibold py-2">Log In</a><a href="/register" class="bg-indigo-600 text-white py-3 rounded-xl font-bold">Create Account</a>{% endif %}</div></div></nav>"""
...
def landing():
    ...
    return render_template_string(f"<!DOCTYPE html><html lang='en'>{BASE_HEAD}<body class='antialiased'>{{{{ navbar|safe }}}}" + """<div class="pt-32 pb-16 lg:pt-48 lg:pb-32 px-6 text-center hero-glow"><div class="max-w-4xl mx-auto"><div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-950/50 border border-indigo-500/30 text-indigo-300 text-xs font-bold mb-8 uppercase tracking-wide"><span class="w-2 h-2 bg-indigo-500 rounded-full animate-pulse"></span> V13.0 Team Edition</div><h1 class="text-4xl sm:text-6xl lg:text-7xl font-bold tracking-tight text-white mb-6 leading-tight">Security for the <br class="hidden sm:block" /><span class="gradient-text">Generative AI Era.</span></h1><p class="text-lg sm:text-xl text-slate-400 mb-10 max-w-2xl mx-auto">Stop employees from accidentally pasting API keys and PII into ChatGPT.</p><div class="flex flex-col sm:flex-row justify-center gap-4 px-4"><a href="/register" class="w-full sm:w-auto px-8 py-4 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-500 transition shadow-lg shadow-indigo-500/25">Start Team Free</a><a href="/guide" class="w-full sm:w-auto px-8 py-4 glass text-slate-300 font-bold rounded-xl hover:bg-slate-800 transition">How to Install</a></div></div></div>{{ footer|safe }}</body></html>""", navbar=render_template_string(NAVBAR_TEMPLATE, logo=LOGO_SVG, current_user=current_user), footer=FOOTER)

@app.route('/guide')
def guide():
    return render_template_string(f"<!DOCTYPE html><html lang='en'>{BASE_HEAD}<body class='bg-slate-950 pb-20'>{{{{ navbar|safe }}}}<main class='pt-32 max-w-4xl mx-auto px-6'><h1 class='text-3xl md:text-5xl font-bold text-white mb-6 text-center'>How to install <span class='gradient-text'>VIGIL Shield</span></h1><div class='space-y-4'><div class='glass p-6 rounded-2xl border-t border-white/5'><h3 class='font-bold text-white text-lg'>1. Download</h3><p class='text-slate-400 text-sm'>Download the .zip file of the extension.</p></div><div class='glass p-6 rounded-2xl border-t border-white/5'><h3 class='font-bold text-white text-lg'>2. Load</h3><p class='text-slate-400 text-sm'>Load unpacked in chrome://extensions.</p></div><div class='glass p-6 rounded-2xl border-t border-white/5'><h3 class='font-bold text-white text-lg'>3. Connect</h3><p class='text-slate-400 text-sm'>Enter your API Key.</p></div></div></main>{{ footer|safe }}</body></html>""", navbar=render_template_string(NAVBAR_TEMPLATE, logo=LOGO_SVG, current_user=current_user), footer=FOOTER)
...
def dashboard():
    ...
    # --- TEAM LOGIC ---
    # Fetch logs for ME + anyone who lists ME as their manager
    cur.execute("""
        SELECT t.*, u.username as employee_name 
        FROM transactions_v7 t
        JOIN users_v7 u ON t.user_id = u.id
        WHERE t.user_id = %s OR t.user_id IN (SELECT id FROM users_v7 WHERE manager_id = %s)
    ...
    # Team Stats
    cur.execute("""
        SELECT COUNT(*) FROM transactions_v7 t 
        WHERE t.user_id = %s OR t.user_id IN (SELECT id FROM users_v7 WHERE manager_id = %s)
    """, (current_user.id, current_user.id))
    ...
    cur.execute("""
        SELECT COUNT(*) FROM transactions_v7 t 
        WHERE (t.user_id = %s OR t.user_id IN (SELECT id FROM users_v7 WHERE manager_id = %s)) AND status='BLOCKED'
    """, (current_user.id, current_user.id))
...
def export_logs():
    conn = get_db_connection(); cur = conn.cursor()
    # Export TEAM logs
    cur.execute("""SELECT t.created_at, u.username, t.source, t.status, t.risk_score, t.risk_reason, t.description 
                   FROM transactions_v7 t JOIN users_v7 u ON t.user_id = u.id 
                   WHERE t.user_id = %s OR t.user_id IN (SELECT id FROM users_v7 WHERE manager_id = %s) ORDER BY t.created_at DESC""", (current_user.id, current_user.id))
    rows = cur.fetchall(); cur.close(); conn.close()
...
def register():
    ...
    if request.method == 'POST':
        ...
        manager_id = None
        if invite:
            cur.execute("SELECT id FROM users_v7 WHERE invite_code = %s", (invite,))
            res = cur.fetchone()
            if res: manager_id = res[0]
            else: msg = "Invalid Invite Code (Account created as Admin)"

        try: 
            cur.execute("INSERT INTO users_v7 (username, password_hash, api_key, invite_code, manager_id) VALUES (%s, %s, %s, %s, %s) RETURNING id", (u, h, k, my_invite, manager_id))
